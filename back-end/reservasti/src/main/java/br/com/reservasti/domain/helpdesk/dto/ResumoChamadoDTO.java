...
public record ResumoChamadoDTO(
    ...
    public ResumoChamadoDTO(Chamado chamado) {
        this(
                chamado.getId(),chamado.getSolicitante().getNomeCompleto(),chamado.getTipoProblema().name(),chamado.getLocalizacao(),chamado.getStatus().name(),chamado.getDataAbertura()
        );
    }
}